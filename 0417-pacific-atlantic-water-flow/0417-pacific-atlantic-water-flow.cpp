    ...
    vector<vector<bool> > bfs(queue<pair<int, int> >&qu ) {
         ...
         while(not qu.empty()){
            ...
            for(int d =0;d<4;d++){
                ...
                if(newRow <0 or newCol < 0 or newRow >= rows or newCol >= cols)continue;// you exited the gride
                if(visited[newRow][newCol]) continue;
                if(h[newRow][newCol]< h[i][j]) continue; // h[newRow][newCol]->neighbours height,h[i][j]->
